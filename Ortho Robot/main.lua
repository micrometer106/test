...
function loadmap(name)
	...
	transmitpos[8] = {-1,-1,-1}
	transmitpos[9] = {-1,-1,-1}
	transmitpos[10] = {-1,-1,-1}
	nowtransmitpos = {}
	nowtransmitpos[1] = {1,1,1}
	transmitornot = false
			...
			for x = 1, mapwidth do
				...
				for i = 1, #tilecolors do
					if r == tilecolors[i][1] and g == tilecolors[i][2] and b == tilecolors[i][3] then
						...
						if i == 6 then
							if transmitpos[1][1] == -1 and transmitpos[1][1] == -1 and transmitpos[1][1] == -1 then
								transmitpos[1] = {x,mapheight-y+1,mapdepth-z+1}
							else
								transmitpos[2] = {x,mapheight-y+1,mapdepth-z+1}
							end
						end

						if i == 7 then
							if transmitpos[3][1] == -1 and transmitpos[3][1] == -1 and transmitpos[3][1] == -1 then
								transmitpos[3] = {x,mapheight-y+1,mapdepth-z+1}
							else
								transmitpos[4] = {x,mapheight-y+1,mapdepth-z+1}
							end
						end

						if i == 8 then
							if transmitpos[5][1] == -1 and transmitpos[5][1] == -1 and transmitpos[5][1] == -1 then
								transmitpos[5] = {x,mapheight-y+1,mapdepth-z+1}
							else
								transmitpos[6] = {x,mapheight-y+1,mapdepth-z+1}
							end
						end

						if i == 9 then
							if transmitpos[7][1] == -1 and transmitpos[7][1] == -1 and transmitpos[7][1] == -1 then
								transmitpos[7] = {x,mapheight-y+1,mapdepth-z+1}
							else
								transmitpos[8] = {x,mapheight-y+1,mapdepth-z+1}
							end
						end

						if i == 10 then
							if transmitpos[9][1] == -1 and transmitpos[9][1] == -1 and transmitpos[9][1] == -1 then
								transmitpos[9] = {x,mapheight-y+1,mapdepth-z+1}
							else
								transmitpos[10] = {x,mapheight-y+1,mapdepth-z+1}
							end
						end
					end
				end
...
function drawbox(x, y, tile)
	...
	if tile.facevisible[side2] then
		...
		--if perspective == "front" and tile.tilenum == 7 then
			--	 local r, g, b, a = unpack(tile.facecolor[3])
			--	lg_setColor(r, g, b, (a or 255)*fillfadecolor)
		--		lg_polygon("fill", newpoint1[1], newpoint1[2], newpoint2[1], newpoint2[2], newpoint5[1], newpoint5[2], newpoint4[1], newpoint4[2])
		--end
	...
	--if tile.tilenum == 7 then
		--if rotation == pi025 and rotation < pi075 then
		--	if drawfaces then
		--		local r, g, b, a = unpack(tile.facecolor[1])
		--		lg_setColor(r, g, b, (a or 255)*fillfadecolor)
		--		lg_polygon("fill", newpoint2[1], newpoint2[2], newpoint3[1], newpoint3[2], newpoint6[1], newpoint6[2], newpoint5[1], newpoint5[2])
		--	end
		--else
		--	if drawfaces then
		--		local r, g, b, a = unpack(tile.facecolor[2])
		--		lg_setColor(r, g, b, (a or 255)*fillfadecolor)
		--		lg_polygon("fill", newpoint2[1], newpoint2[2], newpoint3[1], newpoint3[2], newpoint6[1], newpoint6[2], newpoint5[1], newpoint5[2])
		--	end
	--	end
...
function loadsave()
	if not love.filesystem.exists("save_new.txt") then
		return
	end
	
	local txtcontents = love.filesystem.read("save_new.txt")
...
function savesave()
	...
	love.filesystem.write("save_new.txt", s)
end
